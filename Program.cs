namespace monitorok
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Legalább 2000 pixeles az oszlop: " + masodikFeladat);
            ...
            Console.WriteLine("Ennyi db márkánként:");
            Console.WriteLine(string.Join("\n", negyedikFeladat));

            var ipsDarab = lista.Count(x => x.LCDType == "IPS");
            var tnDarab = lista.Count(x => x.LCDType == "TN");
            var otodikFeladat = ipsDarab == tnDarab ? "Ugyanannyi van." : (ipsDarab > tnDb ? "IPS-ből van több" : "TN-ből van több"); //szerintem így a legegyszerűbb
            Console.WriteLine(otodikFeladat);

            var hatodikFeladat = lista.Where(x => x.Size == 27 && x.Manufacturer != "HP" && x.Manufacturer != "Lenovo").GroupBy(x => x.Manufacturer).Select(x => $"{x.Key}: {x.Count()} db");
            Console.WriteLine("27\"-os monitorok, kivéve HP és Lenovo:");
            Console.WriteLine(string.Join("\n", hatodikFeladat));

            var hetedikFeladat = lista.Where(x => x.LCDType == "IPS").OrderByDescending(x => x.ResolutionX * x.ResolutionY).ThenBy(x => x.Model).Select(x => $"{x.Manufacturer} {x.Model} - {x.ResolutionX}x{x.ResolutionY}");
